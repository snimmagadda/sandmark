...
let render_hotspot output total_samples idx
  ...
  | Some line_counts -> begin
      ...
      try
        fprintf output {| <div id="source_%d" class="row" style="display: none;"><div class="col-xs-offset-1 col-xs-10"> |} idx;
        while true do
          let assoc_opt = List.assoc_opt !current_line line_counts in
          let self_time = match assoc_opt with Some(c) -> c.self_time | None -> 0 in
          let total_time = match assoc_opt with Some(c) -> c.total_time | None -> 0 in
          let src_line = input_line original_src in
          if (!current_line >= min_line) && (!current_line <= max_line) then
            fprintf output {| <div id="source_%d_%d" class="source_code row %s"><div class="col-xs-1 text-right">%d</div><div class="col-xs-9">%s</div><div class="col-xs-1 text-left">%d</div><div class="col-xs-1 text-left">%d</div></div> |} 
            idx
            !current_line
      ...
      with End_of_file -> fprintf output {| </div></div> |}; close_in original_src; else
